...
const versions = ref([
  {
    ...
    tag: '内部测试版',
    highlights: [
      '修复打开已保存草稿后继续编辑并刷新页面时，界面回退到打开草稿前状态的问题',
      '打开草稿后，后续编辑内容现在会作为当前会话独立自动保存，刷新后可正确恢复',
      '支持基于已打开草稿继续修改后另存为新草稿，原草稿内容保持不变',
      '自动保存草稿不再显示在草稿列表中，避免与用户手动保存的草稿混淆'
    ]
  },
  {
    id: 'v0.13',
    ...
    tag: '内部测试版',
    highlights: [
      '修复模板编辑界面中选中默认单元格后，工具栏未正确显示粗体、对齐等当前样式的问题',
      '统一模板编辑与报告编辑界面的测试图片区样式，提升图片上传和 PDF 导出的视觉一致性',
      '修复报告编辑界面页脚“备注：保存部门：”和“保存年限：”输入框在实际输入时仍显示灰色的问题',
      '抽离文本样式读取归一化逻辑，统一工具栏对字重、颜色和对齐方式的识别行为'
    ]
  },
  {
    id: 'v0.12',
    ...
    tag: '内部测试版',
    highlights: [
      '修复模板灰色提示文本样式应用到报告编辑后，正文样式被错误继承的问题',
      '修复报告编辑中修改文本样式后未正确自动保存、刷新后样式丢失的问题',
      '拆分模板占位符样式与报告正文样式存储，避免模板样式覆盖用户编辑结果',
      '优化报告编辑工具栏的样式读取逻辑，确保选中字段时显示正确的正文样式'
    ]
  },
  {
    id: 'v0.11',
