    ...
    sections: [
      {
        title: '缺陷修复',
        items: [
          '修复从草稿列表打开已保存草稿后继续编辑，刷新页面却回到打开草稿前内容的问题',
          '修复继续编辑已打开草稿时，修改内容被隐式写回原草稿，导致原草稿不再保持初始状态的问题',
          '修复自动保存草稿出现在草稿管理列表中，干扰用户区分手动保存草稿的问题'
        ]
      },
      {
        title: '功能优化',
        items: [
          '打开草稿后，会先复制出一个独立的当前编辑会话，后续修改仅作用于当前会话',
          '刷新页面时优先恢复当前会话的自动保存内容，保证打开草稿后的连续编辑状态不丢失',
          '支持以草稿1为基础继续修改后再保存为草稿2，原草稿1仍保持保存当时的内容'
        ]
      },
      {
        title: '技术实现',
        items: [
          '报告自动保存与手动草稿保存拆分为两条独立存储路径，避免刷新恢复与草稿源文件互相覆盖',
          '加载草稿后立即同步到当前模板类型对应的 autosave 会话键，统一后续自动保存入口',
          '后端草稿列表接口过滤 autosave 文件，只保留用户显式保存的草稿记录'
        ]
      }
    ]
  },
