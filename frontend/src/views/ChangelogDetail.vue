    ...
    sections: [
      {
        title: '新功能',
        items: [
          '报告编辑器：支持通用模板、通用英文模板和华为模板的报告编辑功能',
          '模板编辑器：支持自定义 Logo、电子签名、部门章、固定文本、占位文本和表格列宽',
          '格式工具栏：支持字体、字号、颜色、加粗、斜体、下划线和对齐方式设置',
          '动态行管理：测试结果和测试图片区域支持末尾添加、指定插入、末尾删除和指定位置删除',
          '图片上传：支持点击上传、拖拽上传、多图上传和多图布局选择',
          'PDF 导出：支持将报告内容导出为规范的 PDF 文档，并优先使用矢量导出',
          '模板导入导出：支持模板配置文件的导入、导出和复用',
          '自动保存与草稿管理：编辑内容自动保存，支持保存、打开和删除报告草稿'
        ]
      },
      {
        title: '界面特性',
        items: [
          '采用 Element Plus 组件库构建操作界面，提供标签导航和清晰的功能分区',
          '报告编辑页面提供 A4 页面预览，便于在编辑过程中确认最终排版效果',
          '模板编辑与报告编辑保持一致的视觉和交互逻辑，降低模板维护成本',
          '支持本地离线使用，报告内容、模板配置和草稿数据保存在本地环境中'
        ]
      },
      {
        title: '版本说明',
        items: [
          'V1.0 为可靠性测试报告编辑工具的首个正式发行版本',
          '当前版本整合此前内部测试版本中的报告编辑、模板管理、图片处理、PDF 导出和稳定性优化能力',
          '后续版本将根据实际使用反馈继续完善模板类型、导出效果和数据管理体验'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '缺陷修复',
        items: [
          '修复报告编辑界面导出 PDF 时页底表格内容显示不全的问题，避免最后一行被页面边界或页脚区域裁切',
          '修复点击“应用于报告编辑”后切换到报告编辑界面，首次导出 PDF 会触发两次导出的问题',
          '修复重复导出场景中第一个 PDF 可能使用旧模板实例或旧版式状态导致格式异常的问题'
        ]
      },
      {
        title: '功能优化',
        items: [
          '优化 PDF 导出页脚定位逻辑，使页脚保持在页面底部，同时为正文区域保留明确的可用高度',
          '导出模板现在支持在点击后选择保存位置和文件名称，便于按项目或版本管理模板文件',
          '导出模板前会先静默保存当前编辑内容，确保导出的 JSON 模板包含最新修改'
        ]
      },
      {
        title: '技术实现',
        items: [
          '报告通用模板导出监听改为全局单例注册，并增加全局导出锁，避免 keep-alive 缓存实例重复响应导出事件',
          '导出 PDF 的前端 HTML 样式和后端 WeasyPrint 基础样式同步限制正文高度，减少浏览器布局与 PDF 渲染差异',
          '模板导出接入 File System Access API，支持原生保存对话框；不支持该能力的浏览器会自动回退为普通下载'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '缺陷修复',
        items: [
          '修复报告编辑界面点击一次“导出 PDF”却触发两次导出的问题，导出流程增加并发锁并避免成功导出后继续触发兜底导出',
          '修复测试图片区没有上传图片时，PDF 导出会忽略测试图片区域及其跨页内容的问题，导出时保留整体框架',
          '修复测试图片区空占位提示“点击或拖拽上传图片”出现在 PDF 中的问题，该提示现在仅保留在编辑界面',
          '修复测试图片区已上传图片后，PDF 中只显示图片容器但图片本身无法正常绘制的问题'
        ]
      },
      {
        title: '功能优化',
        items: [
          '测试图片区新增布局弹出菜单，上传多张图片后可通过“布局”控件选择排列方式',
          '3 张图片新增“下方通栏”“上方通栏”“四宫格”等固定网格布局，满足不同报告排版需求',
          '编辑界面与 PDF 导出共用同一套图片布局标识，确保最终导出效果与编辑时的布局选择保持一致'
        ]
      },
      {
        title: '技术实现',
        items: [
          'PDF 导出前会将测试图片重编码为标准 JPEG data URL，降低不同图片格式、EXIF 信息或浏览器编码差异导致的导出兼容风险',
          '导出 HTML 中将复杂的编辑态图片布局重建为 PDF 专用表格结构，避开 WeasyPrint 对 flex/grid 图片布局解析不稳定的问题',
          '扩展 reportStore 中的图片布局枚举与校验逻辑，使新增布局可以参与自动保存、草稿保存和 PDF 导出'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '缺陷修复',
        items: [
          '修复打开已保存草稿后继续编辑、刷新页面却回到草稿打开前状态的问题',
          '修复基于已打开草稿继续编辑时，修改内容被误写回原草稿的问题',
          '修复 Logo 与部门章在模板编辑中设置位置后，应用到报告编辑界面出现横向偏移的问题',
          '修复导出 PDF 后页面整体向右偏移的问题，统一导出版心为居中显示',
          '修复正文不足一页时页脚、备注与签名区在 PDF 中无法贴底的问题'
        ]
      },
      {
        title: '界面优化',
        items: [
          '将报告编辑界面与模板编辑界面的部门章透明效果对调：报告编辑中为半透明，模板编辑中为不透明预览',
          'Logo 与部门章的可移动范围扩展到整张 A4 页面，支持放置到右侧和底部空白区域'
        ]
      },
      {
        title: '技术实现',
        items: [
          '自动保存会话与手动草稿保存彻底分离，草稿列表不再混入 autosave 记录',
          '草稿加载后会先复制为独立编辑会话，后续修改仅写入当前会话，避免污染原草稿',
          '同步补强前端导出样式与后端 WeasyPrint 基础样式，减少编辑界面与最终 PDF 的显示偏差'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '缺陷修复',
        items: [
          '修复模板编辑界面中选中默认文本后，顶部工具栏未正确显示当前粗体、颜色和左对齐等样式的问题',
          '修复报告编辑界面页脚“备注：保存部门：”和“保存年限：”右侧输入框在输入正文后仍沿用灰色占位样式的问题',
          '修复模板与报告编辑工具栏对部分默认字重和浏览器 text-align 返回值识别不一致的问题'
        ]
      },
      {
        title: '界面优化',
        items: [
          '统一模板编辑与报告编辑界面的测试图片区表头、间距和占位框样式，减少两个界面之间的视觉割裂',
          '让模板编辑中的测试图片区更贴近报告编辑时的实际上传布局，便于预估最终 PDF 导出效果',
          '保持测试图片区统一的方形占位结构和浅灰卡片风格，提升后续图片上传体验的一致性'
        ]
      },
      {
        title: '技术实现',
        items: [
          '新增共享的文本样式归一化工具，统一处理字体、颜色、字重和对齐方式读取逻辑',
          '模板工具栏与报告工具栏都改为基于实际渲染样式回显，避免仅依赖显式保存样式导致的初始状态读取偏差',
          '页脚输入框保留灰色占位提示，但将正文颜色恢复为默认黑色，确保提示样式与实际内容样式隔离'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '缺陷修复',
        items: [
          '修复在模板编辑中修改灰色提示文本样式并应用到报告编辑后，报告内输入正文再调整样式时错误继承提示文本样式的问题',
          '修复报告编辑界面中仅修改文本样式时未正确触发自动保存，导致刷新或重新进入后样式丢失的问题',
          '修复报告编辑工具栏在选中占位符状态字段时，将灰色提示文本样式误识别为正文样式的问题'
        ]
      },
      {
        title: '功能优化',
        items: [
          '将报告编辑中的模板占位符样式与用户正文样式彻底隔离，确保占位符只受模板控制、正文只受报告编辑控制',
          '正文样式现在完整参与自动保存与恢复，包括字体、字号、颜色、粗体、斜体、下划线和对齐方式',
          '优化字段重新加载后的样式恢复逻辑，确保模板样式与报告样式按各自职责正确生效'
        ]
      },
      {
        title: '技术实现',
        items: [
          'reportStore 新增模板字段样式与报告字段样式双通道存储，避免同一个 fieldFormats 同时承担两类职责',
          'EditableField 组件改为分别处理 templateFormat、placeholderFormat 和 userFormat，消除样式串用风险',
          'Toolbar 组件改为优先读取报告正文样式，并对占位符状态提供安全回退默认值'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '功能优化',
        items: [
          '新增通用模板共享默认配置，统一管理默认列宽、页眉、页脚、签名区和备注区的基础样式',
          '将模板编辑界面的默认列宽调整为与当前报告编辑界面一致的更合理配置',
          '让报告编辑界面的页眉、签名区和备注区占位文本支持继承模板默认样式'
        ]
      },
      {
        title: '缺陷修复',
        items: [
          '修复默认模板应用到报告编辑页面后，“深圳市欣威智能有限公司”“可靠性实验报告”“记录代码”等文字样式与模板编辑界面不一致的问题',
          '修复默认模板在模板编辑与报告编辑两个界面间列宽不一致的问题，确保信息表、样品表、设备表和结果表显示一致',
          '修复报告编辑中行增删、插入、批量删除等操作仅标记脏状态但未正确触发自动保存的问题'
        ]
      },
      {
        title: '代码整理',
        items: [
          '移除模板编辑子组件中重复的模板加载逻辑，统一由父页面负责初始化',
          '清理报告编辑和模板展示中的未使用变量与重复定义，减少后续维护干扰',
          '将分散在多个组件中的默认值归并到共享配置中，降低后续再次出现样式偏差的风险'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '功能优化',
        items: [
          'PDF导出链路升级为“服务端矢量导出优先”模式，提升文本与表格线条清晰度',
          '保留兼容模式导出能力：当矢量导出不可用时自动回退位图导出',
          '优化导出内容构建策略，降低大文档导出失败风险',
          '提高后端导出请求体容量上限，支持包含更多图片的报告导出'
        ]
      },
      {
        title: '缺陷修复',
        items: [
          '修复测试结果信息表格在导出PDF时“备注”列被错误隐藏的问题',
          '修复脚注区域导出后出现换行错位的问题',
          '修复脚注区域在PDF中的超宽问题，确保内容保持在页面可用宽度内',
          '修复Windows环境下WeasyPrint运行库加载不稳定导致的导出失败问题'
        ]
      },
      {
        title: '技术实现',
        items: [
          '新增矢量导出HTML构建逻辑，导出时自动清理交互控件并保留报告结构',
          '补充导出专用CSS规则，确保备注列显示与脚注单行布局稳定',
          '后端新增WeasyPrint DLL目录探测与加载机制，避免多套GTK运行时冲突',
          'start_server.bat 增加WeasyPrint运行库自动注入，支持一键启动后直接导出'
        ]
      }
    ]
  },
  ...
  'v0.7': {
    ...
    sections: [
      ...
      },
      {
        title: '用户体验',
        items: [
          '提供四种行操作方式：末尾添加、指定插入、末尾删除、指定删除',
          '完善安全机制：删除操作前必须用户确认，保留至少一行约束',
          '优化界面细节：侧边栏标题取消加粗，整体布局更加紧凑'
        ]
      },
      {
        title: '技术实现',
